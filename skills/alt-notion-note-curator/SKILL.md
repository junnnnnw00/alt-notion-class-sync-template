...
Do not split the same explanation into a prose-only note plus a detached slide gallery. A reader should be able to start at the top and continue downward without opening a separate preview toggle. Use each managed slide image exactly once in the flow and avoid repeating the same fact in separate `핵심 개념` and `상세 정리` sections.
