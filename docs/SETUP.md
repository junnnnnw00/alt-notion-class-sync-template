...
- **Slide previews did not refresh:** Confirm that the PDF is in the `수업 자료` Files property, not only embedded in the page body. A successful curation run renders every current PDF page into the curator-owned collapsed `슬라이드 미리보기`; if any current file cannot be read, rendered, uploaded, or written, the prior previews and fingerprint intentionally remain.
